...
ENTITIES_DICT = {'ФИО': [' имя ', ' фамилия ', ' отчество ', ' зовут ', ' обращаться '],
                 ...
                 'ЦЕНА': [' цена ', ' стоимость ', ' стоит ', ' скидка ', ' руб ', ' руб. ', ' рублей ', ' рубля ', ' всего ', ' сумма ']
                }
...
"""## 10. Поиск товаров по словарю V2.0.1"""
...
def File_Visualization(audio_path):
  ...
  separec_Google_timed_visualized(path_wav, List_of_Timing)
  ...
  if len(Cleared_Signal) == 0:
    ...
    if len(textes) == 0:
      ...
      if len(result_nums) == 0:
        ...
        printnums(nums_replacer(text), result_nums) # Чтобы верно подсветить нужно заменить все виды цифр, а вот склеивать для наглядности уже не надо
      ...
      if len(tr_starts) == 0:
        ...
        NER_textes = textes_for_NER(path_wav, en_starts, en_ends)
        ...
        if len(NER_textes[index]) != 0:
          ...
          for n in range(len(NER_textes[index])):
            print('Текст №',n,sep='')
            printmd(NER_textes[index][n], get_person_from_ner([], NER_textes[index][n]))
        ...
        if len(NER_textes[index]) != 0:
          ...
          for n in range(len(NER_textes[index])):
            ...
            if len(result_nums) == 0:
              ...
              printnums(nums_replacer(NER_textes[index][n]), result_nums, color = 'Maroon')
        ...
        if len(NER_textes[index]) != 0:
          ...
          for n in range(len(NER_textes[index])):
            ...
            if len(finded_prods_dict) != 0:
              ...
              printmarkedwords(NER_textes[index][n], prods_list, 'Olive')
        ...
        if len(NER_textes[index]) != 0:
          ...
          for n in range(len(NER_textes[index])):
            ...
            show_me_your_money(extract_money(result_PE, set()), 'Goldenrod')

# Комплексная функция для извлечения сущностей
def Total_Analization(audio_path):
  # Удаление музыкальных фрагментов и автоответчика
  Signal, sr = librosa.load(audio_path)
  Cleared_Signal = cut_all_music(Signal)
  ...
  return {'ТЕКСТ': text, 'КЛАСС': cls, 'ВЕРОЯТНОСТЬ': ver, 'ФИО': names, 'ТЕЛЕФОН': phone_nums, 'АДРЕС': adr_dict, 'ТОВАР': products, 'ЦЕНА': money_set}
  
  # При необходимости глубокого анализа обработки в возвращаемые значения можно добавит NER_textes

# Функция визуализации результатат анализа
def Аnalization_Visualization(Analization_Result, colors = ['blue','red','Maroon','Indigo','Olive','Goldenrod']):
  # Отображение результата предсказания НС
  print('-.'*50+'-')
